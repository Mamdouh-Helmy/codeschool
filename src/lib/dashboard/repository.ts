...
function normalizeSnapshot(snapshot: any): DashboardSnapshot {
  ...
  if (Array.isArray(snapshot.metrics)) {
    normalized.metrics = snapshot.metrics.map((m: any) =>
      m._id ? { ...m, id: String(m._id) } : m
    );
  }
  if (Array.isArray(snapshot.activities)) {
    normalized.activities = snapshot.activities.map((a: any) =>
      a._id ? { ...a, id: String(a._id) } : a
    );
  }
  if (Array.isArray(snapshot.enrollments)) {
    normalized.enrollments = snapshot.enrollments.map((e: any) =>
      e._id ? { ...e, id: String(e._id) } : e
    );
  }
  if (snapshot.content && Array.isArray(snapshot.content.items)) {
    normalized.content = {
      ...snapshot.content,
      items: snapshot.content.items.map((c: any) =>
        c._id ? { ...c, id: String(c._id) } : c
      ),
    };
  }
  // health cards usually simple — ensure id if present
  if (Array.isArray(snapshot.health)) {
    normalized.health = snapshot.health.map((h: any) =>
      h._id ? { ...h, id: String(h._id) } : h
    );
  }
  return normalized as DashboardSnapshot;
}

export const fetchDashboardSnapshot = async (): Promise<
  RepositoryResult<DashboardSnapshot>
> => {
  try {
    ...
    const normalized = normalizeSnapshot(snapshot as DashboardSnapshot);
