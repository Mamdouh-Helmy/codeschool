...
const WelcomePopup: React.FC<WelcomePopupProps> = ({ isOpen, onClose }) => {
  ...
  // ✅ جلب بيانات WelcomePopup من API (نفس طريقة Hero)
  useEffect(() => {
    if (!isOpen) return;
    ...
    const fetchWelcomeData = async () => {
      try {
        ...
        if (result.success && result.data.length > 0) {
          ...
          // ✅ التحقق من وجود بيانات Welcome على الأقل
          const hasWelcomeData = (locale === "ar" && data.welcomeTitleAr) || 
                                 (locale === "en" && data.welcomeTitleEn);
          
          if (hasWelcomeData) {
            setSiteContent(data);
          } else {
            // ✅ لا توجد بيانات Welcome، أغلق البوب أب
            setSiteContent(null);
            onClose();
          }
        } else {
          // ✅ لا توجد بيانات أصلاً، أغلق البوب أب
          setSiteContent(null);
          onClose();
        }
      } catch (error) {
  ...
  // ✅ الحصول على البيانات بناءً على اللغة الحالية (نفس منطق Hero)
  ...
  // ✅ إنشاء الـ slides بناءً على البيانات المتاحة
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 md:gap-6 text-xs sm:text-sm text-SlateBlueText dark:text-darktext">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="font-medium">
                        {content.happyParents} {locale === "ar" ? "أولياء أمور راضون" : "Happy Parents"}
                      </span>
                    </div>
                  </div>
                </motion.div>
