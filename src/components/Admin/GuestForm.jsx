// components/Admin/GuestForm.jsx
"use client";
...
export default function GuestForm({ initial, isCreating, onClose, onSaved }) {
  const { t } = useI18n();
  ...
  const isPromotingToInstructor = !isCreating && form.role === "instructor" && initial?.role !== "instructor";
  ...
  const validate = () => {
    if (!form.name.trim()) {
      toast.error(t("guestForm.nameRequired") || "الاسم مطلوب");
      return false;
    }
    if (isCreating && !form.email.trim()) {
      toast.error(t("guestForm.emailRequired") || "الإيميل مطلوب");
      return false;
    }
    if (isCreating && !form.password) {
      toast.error(t("guestForm.passwordRequired") || "الباسورد مطلوب");
      return false;
    }
    if (form.password && form.password !== form.passwordConfirm) {
      toast.error(t("guestForm.passwordMismatch") || "الباسورد مش متطابق");
      return false;
    }
    if (form.password && form.password.length < 6) {
      toast.error(t("guestForm.passwordTooShort") || "الباسورد لازم يكون 6 أحرف على الأقل");
      return false;
    }
    return true;
  };
  ...
  const buildPayload = () => ({
    ...
    // الإيميل بيتبعت وقت الإنشاء بس — read-only وقت التعديل
    ...(isCreating && { email: form.email.trim() }),
    ...(form.username.trim() && { username: form.username.trim() }),
    ...(isCreating && form.password && { password: form.password }),
    ...(!isCreating && { isActive: form.isActive }),
    ...(!isCreating && form.role !== initial?.role && { role: form.role }),
  });
  ...
  const submit = async (e) => {
    ...
    const toastId = toast.loading(
      isCreating ? t("guestForm.creating") || "جاري الإنشاء..." : t("guestForm.updating") || "جاري الحفظ..."
    );

    try {
      const url = isCreating ? "/api/guest" : `/api/guest/${initial._id}`;
      ...
      if (!res.ok || !result.success) {
        throw new Error(
          result.message || (isCreating ? t("guestForm.createFailed") : t("guestForm.updateFailed")) || "حصل خطأ"
        );
      }

      toast.success(
        isCreating
          ? t("guestForm.createdSuccess") || "تم إنشاء الجيست بنجاح"
          : t("guestForm.updatedSuccess") || "تم الحفظ بنجاح",
        { id: toastId }
      );
      onSaved();
      onClose();
      ...
      toast.error(err.message || t("guestForm.updateError") || "حصل خطأ أثناء الحفظ", { id: toastId });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-15 font-semibold text-MidnightNavyText dark:text-white">
              {t("guestForm.basicInfo") || "البيانات الأساسية"}
            </h3>
            <p className="text-12 text-SlateBlueText dark:text-darktext">
              {t("guestForm.basicInfoDescription") || "بيانات الحساب الأساسية"}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-13 font-medium text-MidnightNavyText dark:text-white">
              ...
              {t("guestForm.email") || "الإيميل"} {isCreating && "*"}
            </label>
            <input
              type="email"
              value={form.email}
            ...
            {!isCreating && (
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {t("guestForm.emailReadOnly") || "الإيميل مينفعش يتعدل بعد الإنشاء"}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-13 font-medium text-MidnightNavyText dark:text-white">
              ...
              {t("guestForm.username") || "اليوزرنيم"}
            </label>
            <input
              type="text"
              value={form.username}
            ...
            />
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {t("guestForm.usernameHint") || "لو سبته فاضي وقت الإنشاء هيتولد تلقائي من الاسم"}
            </p>
          </div>
          ...
          {!isCreating && (
            <div className="flex items-center justify-between px-3 py-2.5 border border-PowderBlueBorder dark:border-dark_border rounded-lg">
              <span className="text-13 font-medium text-MidnightNavyText dark:text-white">
                {t("guestForm.isActive") || "الحساب فعّال"}
              </span>
              <button
                type="button"
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-2 text-13 font-medium text-MidnightNavyText dark:text-white">
              ...
              {t("guestForm.imageUrl") || "صورة البروفايل"}
            </label>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {uploadingImage ? (
                      <>
                        ...
                        {t("guestForm.uploading") || "جاري الرفع..."}
                      </>
                    ) : (
                      <>
                        <Upload className="w-4 h-4" />
                        {form.image ? t("guestForm.changeImage") || "تغيير الصورة" : t("guestForm.uploadImage") || "رفع صورة"}
                      </>
                    )}
                    <input
                      type="file"
                  ...
                  {form.image && !uploadingImage && (
                    ...
                    >
                      ...
                      {t("guestForm.removeImage") || "حذف"}
                    </button>
                  )}
                </div>

                <p className="text-11 text-SlateBlueText dark:text-darktext">
                  {t("guestForm.imageRequirements") || "الحد الأقصى: 5MB • JPEG, PNG, WebP — بدون صورة هيتاخد الأفتار الافتراضي"}
                </p>
              </div>
      ...
      {isCreating && (
        <div className="space-y-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl p-5 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-15 font-semibold text-MidnightNavyText dark:text-white">
                {t("guestForm.setPassword") || "الباسورد"}
              </h3>
              <p className="text-12 text-SlateBlueText dark:text-darktext">
                {t("guestForm.passwordRequired") || "الباسورد مطلوب وقت إنشاء الجيست"}
              </p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-13 font-medium text-gray-700 dark:text-gray-300">
                {t("guestForm.confirmPassword") || "تأكيد الباسورد"} *
              </label>
              <input
                type="password"
          ...
          <div className="text-xs text-gray-600 dark:text-gray-400 bg-white/50 dark:bg-gray-800/30 p-3 rounded border border-blue-100 dark:border-blue-900/30">
            <p className="font-medium">{t("guestForm.minChars") || "6 أحرف على الأقل"}</p>
          </div>
        </div>
      )}

      {/* ── Role / Promotion (edit mode only) ── */}
      {!isCreating && (
        <div className="space-y-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl p-5 border border-blue-200 dark:border-blue-800">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center shadow">
            ...
            <div>
              <h3 className="text-15 font-semibold text-MidnightNavyText dark:text-white">
                {t("guestForm.roleTitle") || "الدور / الترقية"}
              </h3>
              <p className="text-12 text-SlateBlueText dark:text-darktext">
                {t("guestForm.roleDescription") || "غيّر دور الحساب من Guest لـ Student أو Instructor"}
              </p>
            </div>
          </div>
          ...
          {isPromotingToInstructor && (
            <div className="flex items-start gap-2 text-xs bg-white/60 dark:bg-gray-800/40 p-3 rounded border border-blue-100 dark:border-blue-900/30">
              ...
              <p className="text-gray-600 dark:text-gray-300">
                {t("guestForm.instructorPromotionNote") ||
          ...
          >
            {loading ? (
              ...
              <>
                ...
                {isCreating ? t("guestForm.create") || "إنشاء جيست" : t("guestForm.saveChanges") || "حفظ التعديلات"}
              </>
            )}
          </button>
        </div>
