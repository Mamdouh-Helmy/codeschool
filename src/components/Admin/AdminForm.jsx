...
export default function AdminForm({ initial, isCreating, onClose, onSaved }) {
    const { t } = useI18n();
    ...
    const uploadImageToCloudinary = async (base64Image) => {
        ...
        try {
            const response = await fetch('/api/upload-image', {
                ...
                body: JSON.stringify({
                    image: base64Image,
                    folder: 'admins' // مجلد خاص بصور المسؤولين
                })
            });
    ...
    const submit = async (e) => {
        ...
        // Validation
        if (!form.name.trim()) {
            toast.error(t("adminForm.nameRequired") || "Name is required");
            return;
        }

        if (isCreating && !form.email.trim()) {
            toast.error(t("adminForm.emailRequired") || "Email is required");
            return;
        }

        if (isCreating && !form.password) {
            toast.error(t("adminForm.passwordRequired") || "Password is required");
            return;
        }

        if (form.password && form.password !== form.passwordConfirm) {
            toast.error(t("adminForm.passwordMismatch") || "Passwords do not match");
            return;
        }

        if (form.password && form.password.length < 6) {
            toast.error(t("adminForm.passwordTooShort") || "Password must be at least 6 characters");
            return;
        }
        ...
        const toastId = toast.loading(
            isCreating
                ? t("adminForm.creating") || "Creating admin..."
                : t("adminForm.updating") || "Updating admin..."
        );
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(result.message || (isCreating ? t("adminForm.createFailed") : t("adminForm.updateFailed")));
            }

            if (result.success) {
                toast.success(
                    isCreating
                        ? t("adminForm.createdSuccess") || "Admin created successfully"
                        : t("adminForm.updatedSuccess") || "Admin updated successfully",
                    { id: toastId }
                );
                onSaved();
                onClose();
            ...
            toast.error(err.message || t("adminForm.updateError") || "An error occurred", { id: toastId });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <form onSubmit={submit} className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-15 font-semibold text-MidnightNavyText dark:text-white">
                            {t("adminForm.basicInfo") || "Basic Information"}
                        </h3>
                        <p className="text-12 text-SlateBlueText dark:text-darktext">
                            {t("adminForm.basicInfoDescription") || "Enter admin details"}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="block text-13 font-medium text-MidnightNavyText dark:text-white flex items-center gap-2">
                            ...
                            {t("adminForm.name") || "Name"} *
                        </label>
                        <input
                            type="text"
                            value={form.name}
                            onChange={(e) => onChange('name', e.target.value)}
                            placeholder={t("adminForm.namePlaceholder") || "Enter full name"}
                    ...
                    <div className="space-y-2">
                        <label className="block text-13 font-medium text-MidnightNavyText dark:text-white flex items-center gap-2">
                            ...
                            {t("adminForm.email") || "Email"} {isCreating && "*"}
                        </label>
                        <input
                            type="email"
                            value={form.email}
                        ...
                        {!isCreating && (
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                {t("adminForm.emailReadOnly") || "Email cannot be changed"}
                            </p>
                        )}
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                            {t("adminForm.usernameHint") || "Letters, numbers, and underscores only. Leave empty to auto-generate."}
                        </p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <label className="block text-13 font-medium text-MidnightNavyText dark:text-white flex items-center gap-2">
                            ...
                            {t("adminForm.imageUrl") || "Profile Image"}
                        </label>
                                ...
                                <div className="flex gap-2">
                                    <label className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-13 font-medium transition-colors border ${uploadingImage
                                        ...
                                        {uploadingImage ? (
                                            <>
                                                ...
                                                {t('adminForm.uploading') || "جاري الرفع..."}
                                            </>
                                        ) : (
                                            <>
                                                <Upload className="w-4 h-4" />
                                                {form.image ? (t('adminForm.changeImage') || "تغيير الصورة") : (t('adminForm.uploadImage') || "رفع صورة")}
                                            </>
                                        )}
                                        <input
                                            type="file"
                                    ...
                                    {form.image && !uploadingImage && (
                                        ...
                                        >
                                            ...
                                            {t('adminForm.removeImage') || "حذف"}
                                        </button>
                                    )}
                                </div>

                                <div className="text-11 text-SlateBlueText dark:text-darktext">
                                    {t('adminForm.imageRequirements') || "الحد الأقصى: 5MB • JPEG, PNG, WebP"}
                                </div>
                            </div>
            ...
            {/* Password Section */}
            <div className="space-y-4 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/10 dark:to-orange-900/10 rounded-xl p-5 border border-red-200 dark:border-red-800">
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-red-500 to-orange-600 rounded-lg flex items-center justify-center shadow">
                        <Lock className="w-4 h-4 text-white" />
                    </div>
                    <div>
                        <h3 className="text-15 font-semibold text-MidnightNavyText dark:text-white">
                            {isCreating
                                ? t("adminForm.setPassword") || "Set Password"
                                : t("adminForm.changePassword") || "Change Password"
                            }
                        </h3>
                        <p className="text-12 text-SlateBlueText dark:text-darktext">
                            {isCreating
                                ? t("adminForm.passwordRequired") || "Password is required for new admin"
                                : t("adminForm.passwordOptional") || "Leave blank to keep current password"
                            }
                        </p>
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <label className="block text-13 font-medium text-gray-700 dark:text-gray-300">
                            {isCreating
                                ? t("adminForm.password") || "Password"
                                : t("adminForm.newPassword") || "New Password"
                            } {isCreating && "*"}
                        </label>
                        <input
                            type="password"
                            ...
                            placeholder="••••••••"
                            className="w-full px-3 py-2.5 border border-red-200 dark:border-red-800 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent dark:bg-dark_input dark:text-white bg-white/50"
                            required={isCreating}
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="block text-13 font-medium text-gray-700 dark:text-gray-300">
                            {t("adminForm.confirmPassword") || "Confirm Password"} {isCreating && "*"}
                        </label>
                        <input
                            type="password"
                ...
                <div className="text-xs text-gray-600 dark:text-gray-400 bg-white/50 dark:bg-gray-800/30 p-3 rounded border border-red-100 dark:border-red-900/30">
                    <p className="font-medium mb-1">{t("adminForm.passwordRequirements") || "Password Requirements:"}</p>
                    <ul className="space-y-1 list-disc list-inside">
                        <li>{t("adminForm.minChars") || "At least 6 characters"}</li>
                        {!isCreating && <li>{t("adminForm.leaveBlank") || "Leave blank to keep current password"}</li>}
                    </ul>
                </div>
            </div>
                    ...
                    >
                        {loading ? (
                            <>
                                <Loader2 className="w-4 h-4 animate-spin" />
                                {isCreating
                                    ? t("adminForm.creating") || "Creating..."
                                    : t("adminForm.updating") || "Updating..."
                                }
                            </>
                        ) : (
                            <>
                                ...
                                {isCreating
                                    ? t("adminForm.create") || "Create Admin"
                                    : t("adminForm.saveChanges") || "Save Changes"
                                }
                            </>
                        )}
                    </button>
