...
type ModalProps = {
  ...
  onClose: () => void;
  size?: ModalSize;
  noPadding?: boolean; // ← الجديد: يشيل الـ header والـ padding عشان الـ child يتحكم بالكامل
};
...
const sizeClasses: Record<ModalSize, string> = {
  ...
  lg:   "max-w-lg",
  xl:   "max-w-3xl",
  full: "max-w-[95vw]",   // ← اتوسع من max-w-6xl
};
...
export default function Modal({
  ...
  const modalContent = (
    ...
    >
      <div
        className={`
          bg-white dark:bg-darkmode rounded-2xl shadow-2xl w-full
          ${sizeClasses[size]}
          ${noPadding ? "overflow-hidden" : "max-h-[85vh] overflow-hidden border border-PowderBlueBorder dark:border-dark_border"}
        `}
        onClick={e => e.stopPropagation()}
      >
        {noPadding ? (
          // ── الـ child يتحكم في كل شئ (header + scroll + footer) ──
          children
        ) : (
          <>
            {/* Default header */}
