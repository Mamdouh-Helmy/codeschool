  ...
  return (
    <section style={{ background: dark ? C.darkmode : "#fff", padding:"6px 0 8px" }}>
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"0 20px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(12,1fr)", alignItems:"center", gap:30 }}
          dir={isRTL ? "rtl" : "ltr"}>
          ...
          {/* Images col */}
          <div style={{ gridColumn:"span 6", display:"flex", alignItems:"center", gap:"clamp(8px, 3vw, 12px)" }}
            ...
            <div style={{ position:"relative",flex:1 }}>
              ...
              {data[`instructor1${s}`] && (
                <div style={{ position:"absolute",top:16,zIndex:50,[isRTL?"right":"left"]:0,transform:isRTL?"translateX(33%)":"translateX(-33%)",background:C.primary,borderRadius:16,boxShadow:"0 6px 20px "+C.primary+"55",padding:"clamp(8px, 2vw, 12px) clamp(12px, 3vw, 16px)" }} className="instructor-card">
            ...
            <div style={{ position:"relative",flex:1 }}>
              <EditableImage src={data.secondImageUrl} alt={data.secondImageAlt}
                ...
                style={{ width:"100%",height:"clamp(250px, 50vw, 400px)",borderRadius:r2,marginTop:"clamp(60px, 15vw, 128px)",background:C.primary,overflow:"hidden" }}/>
              {data[`instructor2${s}`] && (
                <div style={{ position:"absolute",top:"clamp(180px, 35vw, 272px)",zIndex:50,[isRTL?"left":"right"]:0,transform:isRTL?"translateX(-33%)":"translateX(33%)",background:"#ffbd59",borderRadius:16,boxShadow:"0 6px 20px #ffbd5966",padding:"clamp(8px, 2vw, 12px) clamp(12px, 3vw, 16px)" }} className="instructor-card-2">
                  <p style={{ fontSize:"clamp(12px, 3.5vw, 16px)",fontWeight:700,color:"#fff",whiteSpace:"nowrap",margin:0 }}>
      ...
      <style jsx>{`
        @media (max-width: 768px) {
          .hero-text-col { grid-column: span 12 !important; text-align: ${isRTL ? 'right' : 'left'} !important; }
          .hero-images-col { grid-column: span 12 !important; }
          .instructor-card { transform: ${isRTL ? 'translateX(20%)' : 'translateX(-20%)'} !important; }
          .instructor-card-2 { transform: ${isRTL ? 'translateX(-20%)' : 'translateX(20%)'} !important; }
        }
      `}</style>
    </section>
  );
...
function PopupTemplate({ data, lang, dark, set, uploadImage, uploading }) {
  ...
  return (
    <div dir={isRTL?"rtl":"ltr"} style={{ display:"flex",flexDirection:"column",borderRadius:22,overflow:"hidden",boxShadow:dark?"0 24px 64px rgba(0,0,0,.5)":"0 24px 64px rgba(0,0,0,.14)",minHeight:"clamp(500px, 80vh, 650px)",position:"relative",background:panelBg }} className="popup-container">
      <div style={{ position:"absolute",top:10,[isRTL?"left":"right"]:10,zIndex:20,width:36,height:36,borderRadius:"50%",background:dark?"rgba(255,255,255,.1)":"rgba(255,255,255,.9)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,.15)",cursor:"pointer" }}>
        <X size={16} color={C.SlateBlue}/>
      </div>

      {/* Left panel */}
      <div style={{ width:"100%",background:`linear-gradient(135deg,${C.primary} 0%,#7a45e6 50%,#6a3ac9 100%)`,padding:"clamp(20px, 5vw, 32px) clamp(16px, 4vw, 20px)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",minHeight:"clamp(400px, 60vh, 500px)" }} className="popup-left">
        ...
        <div style={{ background:"rgba(255,255,255,.2)",backdropFilter:"blur(8px)",borderRadius:"50%",width:"clamp(50px, 12vw, 64px)",height:"clamp(50px, 12vw, 64px)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,boxShadow:"0 4px 16px rgba(0,0,0,.15)" }}>
          <SlideIcon size={32} color="#fff"/>
        </div>
        <div style={{ background:"rgba(255,255,255,.1)",backdropFilter:"blur(8px)",borderRadius:22,overflow:"hidden",width:"clamp(120px, 30vw, 160px)",height:"clamp(120px, 30vw, 160px)",marginBottom:20,border:"1px solid rgba(255,255,255,.25)",padding:12 }}>
        ...
        <div style={{ width:"100%",maxWidth:320,display:"flex",flexDirection:"column",gap:10 }}>
          {fKeys.map((key,i)=>(
            <div key={key} style={{ display:"flex",alignItems:"center",gap:12,background:"rgba(255,255,255,.2)",backdropFilter:"blur(4px)",borderRadius:14,padding:"clamp(8px, 2vw, 10px) clamp(12px, 3vw, 16px)",border:"1px solid rgba(255,255,255,.3)",direction:isRTL?"rtl":"ltr" }}>
      ...
      {/* Right panel */}
      <div style={{ flex:1,padding:"clamp(24px, 6vw, 32px) clamp(20px, 5vw, 28px)",background:panelBg,display:"flex",flexDirection:"column",justifyContent:"center",textAlign:isRTL?"right":"left",overflowY:"auto" }} className="popup-right">
        ...
        {slide===1&&(
          <div style={{ background:`linear-gradient(90deg,${C.primary},#ffbd59)`,color:"#fff",borderRadius:14,padding:"clamp(10px, 3vw, 12px) clamp(12px, 4vw, 16px)",display:"flex",alignItems:"center",gap:16,marginBottom:16,boxShadow:"0 6px 20px "+C.primary+"44",flexWrap:"wrap" }}>
        ...
        <button style={{ width:"100%",background:"#25D366",color:"#fff",border:"none",borderRadius:14,padding:"clamp(12px, 3vw, 14px) clamp(16px, 4vw, 20px)",fontSize:"clamp(13px, 3.5vw, 15px)",fontWeight:600,marginBottom:12,cursor:"default",display:"flex",alignItems:"center",justifyContent:"center",gap:12,boxShadow:"0 4px 14px #25D36644" }}>
          <FaWhatsapp size={22}/>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"clamp(8px, 2vw, 12px)",marginBottom:24 }}>
          <button style={{ background:C.ElectricAqua,color:"#fff",border:"none",borderRadius:14,padding:"clamp(10px, 2.5vw, 12px) clamp(12px, 3vw, 16px)",fontSize:"clamp(12px, 3vw, 14px)",fontWeight:600,cursor:"default",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 4px 12px "+C.ElectricAqua+"44" }}>
            <Phone size={17}/> {isRTL?"اتصل الآن":"Call Now"}
          </button>
          <button style={{ background:C.primary,color:"#fff",border:"none",borderRadius:14,padding:"clamp(10px, 2.5vw, 12px) clamp(12px, 3vw, 16px)",fontSize:"clamp(12px, 3vw, 14px)",fontWeight:600,cursor:"default",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 4px 12px "+C.primary+"44" }}>
            <Mail size={17}/> {isRTL?"راسلنا عبر الإيميل":"Email Us"}
          </button>
        </div>
        <div style={{ paddingTop:20,borderTop:"1px solid "+divLine,display:"flex",alignItems:"center",justifyContent:"center",gap:"clamp(12px, 3vw, 16px)",flexWrap:"wrap" }}>
      ...
      <style jsx>{`
        @media (min-width: 768px) {
          .popup-container { flex-direction: ${isRTL ? 'row-reverse' : 'row'} !important; }
          .popup-left { width: 42% !important; }
        }
      `}</style>
    </div>
  );
...
export default function HeroAdminLiveEditor() {
  ...
  // ── تحميل البيانات من الداتابيز عند الفتح ──
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // في حالة فشل التحميل، يبقى DEFAULT
      }
    };
    load();
  }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      const url    = payload._id
        ? `/api/section-images-hero/${payload._id}`
  ...
  return (
    <div style={{ minHeight:"100vh", background:frameBg, fontFamily:"'Segoe UI',Tahoma,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      {/* ══ TOP BAR ══ */}
      <div style={{ background:topBg,minHeight:58,padding:"clamp(8px, 2vw, 12px) clamp(16px, 4vw, 22px)",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 20px rgba(0,0,0,.35)",flexShrink:0,flexWrap:"wrap",gap:"10px" }}>
        <div style={{ display:"flex",alignItems:"center",gap:"clamp(8px, 2vw, 12px)" }}>
          <div style={{ width:"clamp(32px, 8vw, 36px)",height:"clamp(32px, 8vw, 36px)",borderRadius:10,background:`linear-gradient(135deg,${C.primary},#ffbd59)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"clamp(16px, 4vw, 18px)",boxShadow:`0 4px 12px ${C.primary}66` }}>⚡</div>
          <div>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"clamp(8px, 2vw, 10px)",flexWrap:"wrap" }}>
          {toast && (
            <div style={{ background:toast.type==="err"?"#ef4444":"#10b981",color:"#fff",padding:"clamp(4px, 1.5vw, 6px) clamp(10px, 3vw, 14px)",borderRadius:8,fontSize:"clamp(10px, 2.5vw, 12px)",fontWeight:600,animation:"slideIn .25s ease",display:"flex",alignItems:"center",gap:6 }}>
          ...
          <div style={{ display:"flex",alignItems:"center",background:"rgba(255,255,255,.1)",borderRadius:8,padding:3,gap:2 }}>
            {[{id:"ar",flag:"🇸🇦"},{id:"en",flag:"🇺🇸"}].map(({id,flag})=>(
              <button key={id} onClick={()=>{ set("language",id); setPLang(id); }}
                style={{ background:data.language===id?"rgba(140,82,255,0.6)":"transparent",border:"none",borderRadius:6,padding:"4px 10px",cursor:"pointer",color:"#fff",fontSize:"clamp(10px, 2.5vw, 12px)",fontWeight:data.language===id?700:400,transition:"all .15s",display:"flex",alignItems:"center",gap:4 }}>
                <span>{flag}</span> {id.toUpperCase()}
              </button>
            ))}
          </div>

          <button onClick={()=>set("isActive",!data.isActive)} style={{ background:data.isActive?"#10b98122":"rgba(255,255,255,.1)",border:`1px solid ${data.isActive?"#10b981":"rgba(255,255,255,.2)"}`,borderRadius:8,padding:"clamp(5px, 1.8vw, 7px) clamp(10px, 3vw, 13px)",cursor:"pointer",color:data.isActive?"#10b981":"rgba(255,255,255,.6)",fontSize:"clamp(10px, 2.5vw, 12px)",fontWeight:700,display:"flex",alignItems:"center",gap:6 }}>
            {data.isActive?<Eye size={14}/>:<EyeOff size={14}/>}
            {data.isActive?"مفعّل":"معطّل"}
          </button>

          <button onClick={()=>setDark(d=>!d)} style={{ background:"rgba(255,255,255,.12)",border:"none",borderRadius:8,padding:"clamp(5px, 1.8vw, 7px) clamp(10px, 3vw, 13px)",cursor:"pointer",color:"#fff",display:"flex",alignItems:"center",gap:6 }}>
            {dark?<Sun size={15}/>:<Moon size={15}/>}
            <span style={{ fontSize:"clamp(10px, 2.5vw, 11px)",fontWeight:600 }}>{dark?"Light":"Dark"}</span>
          </button>

          <button onClick={save} disabled={saving} style={{ background:saving?"#6b7280":`linear-gradient(135deg,${C.primary},#7a45e6)`,color:"#fff",border:"none",padding:"clamp(7px, 2vw, 9px) clamp(16px, 5vw, 22px)",borderRadius:10,fontWeight:700,fontSize:"clamp(11px, 3vw, 13px)",cursor:saving?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:7,boxShadow:saving?"none":`0 4px 14px ${C.primary}55` }}>
      ...
      {/* ══ CONTROLS BAR ══ */}
      <div style={{ background:sidebarBg,borderBottom:`0.5px solid ${borderClr}`,padding:"clamp(8px, 2vw, 10px) clamp(16px, 4vw, 20px)",display:"flex",alignItems:"center",gap:"clamp(8px, 2vw, 10px)",flexWrap:"wrap",flexShrink:0 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",background:dark?C.dark_input:"#f1f0f5",borderRadius:10,padding:3,gap:2,border:`0.5px solid ${borderClr}`,flexWrap:"wrap" }}>
          {[
            { id:"hero",  label:"Hero",  icon:<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg> },
            { id:"popup", label:"Popup", icon:<svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 10h18"/></svg> },
          ].map(({ id, label, icon }) => (
        ...
        {/* Edit hint */}
        <div style={{ display:"flex",alignItems:"center",gap:6,padding:"clamp(4px, 1.5vw, 6px) clamp(8px, 2.5vw, 12px)",background:"rgba(140,82,255,0.09)",border:"0.5px solid rgba(140,82,255,0.35)",borderRadius:8,fontSize:"clamp(10px, 2.5vw, 11px)",fontWeight:500,color:C.primary,whiteSpace:"nowrap" }}>
        ...
        {/* Language indicator */}
        <div style={{ display:"flex",alignItems:"center",gap:6,padding:"clamp(4px, 1.5vw, 6px) clamp(8px, 2.5vw, 12px)",background:data.language==="ar"?"rgba(140,82,255,0.09)":"rgba(250,180,70,0.09)",border:`0.5px solid ${data.language==="ar"?"rgba(140,82,255,0.35)":"rgba(250,180,70,0.35)"}`,borderRadius:8,fontSize:"clamp(10px, 2.5vw, 11px)",fontWeight:600,color:data.language==="ar"?C.primary:C.YellowRating }}>
          {data.language==="ar"?"🇸🇦 اللغة المحفوظة: عربي":"🇺🇸 Saved Language: English"}
        </div>

        {/* Live badge */}
        <div style={{ marginLeft:"auto",display:"flex",alignItems:"center",gap:6,fontSize:"clamp(10px, 2.5vw, 11px)",fontWeight:500,color:mutedClr,whiteSpace:"nowrap" }}>
      ...
      {/* ══ PREVIEW AREA ══ */}
      <div style={{ flex:1,overflowY:"auto",padding:"clamp(20px, 5vw, 28px) clamp(16px, 4vw, 32px)",display:"flex",justifyContent:"center",alignItems:"flex-start" }}>
        <div style={{ width:"100%", maxWidth:"1400px", margin:"0 auto" }}>
          <div style={{ background:dark?"#0a0a1a":C.MidnightNavy,borderRadius:"14px 14px 0 0",padding:"clamp(8px, 2vw, 10px) clamp(12px, 3vw, 16px)",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
            <div style={{ width:11,height:11,borderRadius:"50%",background:"#ff5f57" }}/>
            <div style={{ width:11,height:11,borderRadius:"50%",background:"#febc2e" }}/>
            <div style={{ width:11,height:11,borderRadius:"50%",background:"#28c840" }}/>
            <div style={{ flex:1,marginLeft:10,background:"rgba(255,255,255,.1)",borderRadius:6,padding:"3px 12px",color:"rgba(255,255,255,.5)",fontSize:"clamp(9px, 2.5vw, 11px)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
          ...
          <div style={{ background:dark?C.darkmode:"#fff",borderRadius:"0 0 14px 14px",overflow:"visible",boxShadow:"0 24px 60px rgba(0,0,0,.15)",padding:pMode==="popup"?"clamp(20px, 5vw, 28px) clamp(16px, 4vw, 24px)":0 }}>
            {pMode==="hero"
          ...
          <div style={{ marginTop:16,padding:"clamp(10px, 2.5vw, 12px) clamp(14px, 3.5vw, 18px)",background:sidebarBg,borderRadius:10,border:`0.5px solid ${borderClr}`,display:"flex",gap:"clamp(8px, 2vw, 10px)",alignItems:"center",flexWrap:"wrap" }}>
            ...
            <span style={{ fontSize:"clamp(10px, 2.5vw, 11px)",color:mutedClr,lineHeight:1.7 }}>
              ...
              {" "}اللغة المحفوظة حالياً: <b style={{color:C.primary}}>{data.language==="ar"?"العربية":"English"}</b>
            </span>
          </div>
        </div>
      </div>
