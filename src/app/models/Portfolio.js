...
const portfolioSchema = new mongoose.Schema(
  {
    ...
    stats: {
      ...
      codeCommits: { type: Number, min: 0, default: 0 },
    },
    skills: [
      {
    ...
    certificates: [
      {
        title: { type: String, required: [true, "Certificate title is required"], trim: true, maxlength: [150, "Title cannot exceed 150 characters"] },
        description: { type: String, maxlength: [500, "Description cannot exceed 500 characters"], default: "" },
        image: { url: { type: String, default: "" }, alt: { type: String, default: "" } },
    ...
    settings: {
      theme: { type: String, enum: ["light", "dark", "blue", "green"], default: "dark" },
      layout: { type: String, enum: ["standard", "minimal", "creative"], default: "standard" },
    },

    // ✅ جديد: لتتبع آخر مرة اتبعت فيها رسالة التذكير بعدم التحديث
    metadata: {
      lastInactivityReminderSentAt: { type: Date, default: null },
    },
  },
  {
    timestamps: true,
...
portfolioSchema.pre("save", async function () {
  if (this.skills && this.skills.length > 50) throw new Error("Cannot have more than 50 skills");
  if (this.projects && this.projects.length > 100) throw new Error("Cannot have more than 100 projects");
  if (this.certificates && this.certificates.length > 50) throw new Error("Cannot have more than 50 certificates");
  if (this.experience && this.experience.length > 30) throw new Error("Cannot have more than 30 experience entries");
  if (this.education && this.education.length > 20) throw new Error("Cannot have more than 20 education entries");
  if (this.services && this.services.length > 20) throw new Error("Cannot have more than 20 services");
