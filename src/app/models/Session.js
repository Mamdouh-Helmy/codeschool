...
const SessionSchema = new mongoose.Schema({
  ...
  sessionNumber: {
    ...
    validate: {
      ...
      message: 'Session number must be 1, 2, or 3 (Lessons 1-2→S1, 3-4→S2, 5-6→S3)'
    }
  },
...
SessionSchema.pre('save', async function(next) {
  try {
    if (this.isNew) {
      ...
      if (existingSession) {
        const error = new Error(`Session already exists for group ${this.groupId}, module ${this.moduleIndex}, session ${this.sessionNumber}`);
...
SessionSchema.virtual('formattedDate').get(function() {
  return this.scheduledDate ? this.scheduledDate.toISOString().split('T')[0] : '';
});
...
SessionSchema.methods.getDisplayDetails = function() {
  ...
  const lessonsText = this.lessonIndexes.map(idx => `Lesson ${idx + 1}`).join(' & ');
...
SessionSchema.methods.getStatusColor = function() {
  ...
  return colors[this.status] || 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
};
...
SessionSchema.methods.getNextSession = async function() {
  const nextSession = await mongoose.model('Session').findOne({
    groupId: this.groupId,
...
SessionSchema.methods.getPreviousSession = async function() {
  const prevSession = await mongoose.model('Session').findOne({
    groupId: this.groupId,
...
SessionSchema.methods.getModuleSessions = async function() {
  const moduleSessions = await mongoose.model('Session').find({
    groupId: this.groupId,
    moduleIndex: this.moduleIndex,
    isDeleted: false
...
SessionSchema.statics.getSessionsByDay = async function(groupId) {
  ...
  sessions.forEach(session => {
    ...
    const dayName = new Date(session.scheduledDate).toLocaleDateString('en-US', { weekday: 'long' });
    ...
    grouped[dateKey].sessions.push({
      ...
      lessonsText: session.lessonIndexes.map(idx => `Lesson ${idx + 1}`).join(' & ')
    });
  });
...
SessionSchema.statics.getSessionsByDayOfWeek = async function(groupId, dayOfWeek) {
  const sessions = await this.find({ 
    groupId, 
    isDeleted: false 
  }).lean();
...
SessionSchema.statics.getSessionsByModule = async function(groupId) {
  ...
  sessions.forEach(session => {
    ...
    groupedByModule[moduleKey].sessions.push({
      ...
      formattedDate: new Date(session.scheduledDate).toISOString().split('T')[0],
      dayName: new Date(session.scheduledDate).toLocaleDateString('en-US', { weekday: 'long' }),
      startTime: session.startTime,
      endTime: session.endTime,
      status: session.status,
      attendanceTaken: session.attendanceTaken,
  ...
  return Object.values(groupedByModule).sort((a, b) => a.moduleIndex - b.moduleIndex);
};
...
SessionSchema.statics.getSessionsByDateRange = async function(groupId, startDate, endDate) {
  return await this.find({
    groupId,
