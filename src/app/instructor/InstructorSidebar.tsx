...
export default function InstructorSidebar({
  user = null,
  onLogout = () => {},
}: InstructorSidebarProps): React.JSX.Element {
  const { t } = useI18n();
  ...
  const getUserName = (): string => user?.name || (isRTL ? "مدرس" : "Instructor");
  const getUserEmail = (): string => user?.email || "instructor@example.com";
          ...
          {!isCollapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-xs text-gray-500 dark:text-[#8b949e]">
                {isRTL ? "بوابة المدرس" : "Instructor Portal"}
              </span>
            </div>
          )}
        </Link>
