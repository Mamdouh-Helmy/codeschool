...
function HoldBanner({ isAr, lockedSessionsCount, totalActiveSessions }) {
  ...
  // نبني رسالة حسب ما إذا كانت جزئية أم كاملة
  const message = lockedSessionsCount === totalActiveSessions
    ? t(
        "كل جلساتك المتاحة متوقفة مؤقتًا. هتقدر تشتغل عليها بعد فكّ الـ Hold.",
        "All your active sessions are paused. You can work on them once the hold is released."
      )
    : t(
        `${lockedSessionsCount} جلسة من ${totalActiveSessions} متوقفة حاليًا. الجلسات التانية شغالة عادي.`,
        `${lockedSessionsCount} of ${totalActiveSessions} sessions are paused. The rest are working normally.`
      );
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="relative w-full sm:max-w-2xl max-h-[94vh] overflow-y-auto bg-white dark:bg-[#0d1117] rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col border border-gray-100 dark:border-[#21262d]">
        ...
        <div className="p-5 space-y-4 bg-gray-50/50 dark:bg-[#0d1117]">
            ...
            <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
              ...
              <div>
                <p className="text-sm font-black text-amber-800 dark:text-amber-300">
                  {t("الجلسة مقفولة مؤقتًا", "Session is on hold")}
                </p>
          ...
          {!isPartial && isOpenToday && session.isOffline && session.locationInfo && !sessionIsLocked && (
...
function SessionRow({ session, onOpen, isAr, onRequestAccess }) {
  ...
  const handleClick = () => {
    if (sessionIsLocked && !isCompleted) {
      // نعرض المودال بمعاينة فقط لو مقفولة
      onOpen(session);
      return;
    }
    if (isClickable) {
