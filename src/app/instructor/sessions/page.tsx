...
export default function InstructorSessionsPage() {
  ...
  const fetchSessions = async (page = 1) => {
    try {
      ...
      let url = `/api/instructor-dashboard/sessions?page=${page}&limit=${pagination.limit}&sortBy=${sortBy}&sortOrder=${sortOrder}`;
  ...
  const exportToCSV = () => {
    ...
    const data = sessions.map(session => {
      const attendance = session.metadata?.attendanceStats || { total: 0, present: 0, absent: 0, late: 0, excused: 0 };
      const attendanceText = `${attendance.present}/${attendance.absent}/${attendance.late}/${attendance.excused}`;
      ...
      return [
        ...
        session.meetingLink || "لا يوجد",
        session.instructorNotes || "لا توجد ملاحظات"
      ];
    });
    ...
    link.download = `جلسات_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-gray-800 dark:text-white"
                >
                  <option value="scheduledDate">التاريخ</option>
                ...
                <select
                  ...
                  className="w-24 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-gray-800 dark:text-white"
                >
        ...
        <div className="space-y-4">
          {sessions.map((session) => {
            ...
            const attendanceStats = session.metadata?.attendanceStats || { total: 0, present: 0, absent: 0, late: 0, excused: 0 };
        ...
        {sessions.length === 0 && (
          <div className="text-center py-12 bg-white dark:bg-secondary rounded-xl shadow-lg">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">
              {searchTerm || filterStatus !== "all" || filterGroup !== "all" || filterFromDate || filterToDate
